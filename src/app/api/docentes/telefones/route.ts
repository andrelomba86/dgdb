...
export async function GET(request: Request) {
  try {
    ...
    logError('Erro ao buscar telefones do docente', error)
...
//     const conn = await conectarDB()
//     const [result] = await conn.execute('INSERT INTO telefone (docente_id, telefone, tipo) VALUES (?, ?, ?)', [
//       docente_id,
//       telefone,
//       tipo,
//     ])
...
//   } catch (error) {
//     logError('Erro ao salvar telefone', error)
...
//     const conn = await conectarDB()
//     const [result] = await conn.execute('UPDATE telefone SET telefone = ?, tipo = ? WHERE id = ?', [telefone, tipo, id])
//     await conn.end()
...
//     return NextResponse.json(result)
//   } catch (error) {
//     logError('Erro ao excluir telefone', error)
